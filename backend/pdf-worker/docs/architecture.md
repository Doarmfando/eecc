...
`extractors/banco_nacion/` (`banco-nacion-v1`) se escribió sin muestra y se **corrigió con un estado de cuenta real de 144 páginas** el 2026-09-22. La primera versión no reconocía ese documento: lo que sigue describe la plantilla confirmada. Sigue sin apoyarse en posiciones fijas ni en un único juego de rótulos —lee lo que el documento declara de sí mismo— y cualquier cosa que no pueda demostrar deja el resultado en `NEEDS_REVIEW`:

- la cabecera de la tabla —un rótulo de fecha (`FECHA` o `DIA`), uno de detalle (`CODIFICACION`/`DESCRIPCION`/`CONCEPTO`/`DETALLE`/`GLOSA`), uno de saldo (`SALDO`/`SALDOS`) y al menos una columna de dinero— da la posición de `CARGOS`/`DEBITOS`/`RETIROS`, `ABONOS`/`CREDITOS`/`DEPOSITOS`, `IMPORTE`/`MONTO` con signo y `SALDO`; cada importe se asigna a la columna más cercana alineando por la izquierda, por la derecha o por el centro, porque la cabecera puede ir centrada sobre importes alineados a la derecha. La cabecera de la primera página vale para las siguientes;
- **la fecha se busca por su columna, no por su posición en la fila**: el documento real la imprime en la última (`DIA`), no en la primera. Si la cabecera no nombra la columna de fecha, se recurre a la convención de que abra la fila. Solo cuando abre la fila puede traer detrás una segunda fecha valor. Sin año, el año sale del periodo impreso (`DEL 01/06/2026 AL 30/06/2026`, también si cruza de año) o de `default_year`; si no hay ninguno, `BANCO_NACION_DATE_WITHOUT_YEAR`;
...
La detección **no exige el nombre del banco**: el documento real no lo menciona en ninguna parte —ni en cabecera, ni en pie, ni en metadatos—, y exigirlo dejaba el techo de confianza en 0,60 frente a un mínimo de 0,75, así que todo estado de cuenta real acababa en el respaldo genérico, donde falla entero. La firma propia es la cabecera: `CODIFICACION` (0,35), `SALDOS DIA` (0,20) y `NRO CHEQUE` (0,10), más el formato de cuenta `00-000-000000` (0,15) y `SALDO ANTERIOR` (0,10). La marca del banco sigue sumando 0,45 cuando está. `CARGOS | ABONOS | SALDO` por sí solo no reclama nada: lo usan varios bancos. `resolve_best_strategy` lo prueba después de BCP e Interbank.

El Excel usa el esquema `eecc.statement.banco_nacion` (`Resumen`, `Movimientos` con fecha valor, `Control_Paginas`, `Validaciones`) y no exporta titular, documento de identidad ni número de cuenta. `tests/characterization` prueba cualquier PDF real del banco que se deje en `referencias/`; con la muestra de 144 páginas da `SUCCEEDED`, 7.450 movimientos y las cinco invariantes en `PASSED`.

Particularidades de la plantilla real, todas confirmadas contra el documento:

- **el saldo se imprime una vez por día**, no en cada movimiento: la columna se rotula `SALDOS DIA`. La reconciliación ya lo admitía, porque compara solo donde hay saldo impreso y arrastra el resto;
- **el cierre va en dos líneas**: `TOTAL | TOTAL CARGOS | TOTAL ABONOS | SALDO ACTUAL` arriba y los importes alineados debajo, cada uno bajo su rótulo. Es la única plantilla vista que separa rótulo e importe en filas distintas;
- **el saldo final lleva relleno de protección** (`*********12,345.67`), como un cheque. Los asteriscos se quitan antes de leer el importe;
- no hay `VAN`/`VIENEN`: el arrastre entre páginas no se imprime.
