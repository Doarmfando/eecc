"""Adaptador de pdfplumber hacia filas visuales Interbank. No contiene reglas bancarias."""
...
from .models import InterbankPageReadout, InterbankPdfReadResult, InterbankWord

# El concepto, en negro, y los importes, en color, no comparten línea base exacta.
# Dos movimientos están separados unos 30 puntos: 4 agrupa la fila sin mezclar dos.
ROW_TOLERANCE = Decimal("4")
...
) -> tuple[tuple[InterbankWord, ...], ...]:
    ordered = sorted(words, key=lambda word: (word.top, word.x0))
    rows: list[list[InterbankWord]] = []
...
def _read_page(page: pdfplumber.page.Page, page_number: int) -> tuple[InterbankPageReadout, str]:
...
def _read_page_range(arguments: PageRange) -> list[tuple[InterbankPageReadout, str]]:
...
def read_interbank_pdf(pdf_path: Path, *, max_workers: int | None = None) -> InterbankPdfReadResult:
